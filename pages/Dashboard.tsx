...
import { RefreshCw, Scale, FileText, X, ExternalLink, AlertCircle } from 'lucide-react';
import { analyzeLegalText } from '../services/geminiService';
import { AnalysisResult } from '../types';

interface SentenzaData extends AnalysisResult {
  id?: string;
  timestamp?: string;
  status?: string;
  message?: string;
...
const Dashboard: React.FC = () => {
  // Mock fallback data
  const mockJudgment: SentenzaData = {
  ...
  const [currentData, setCurrentData] = useState<SentenzaData | null>(null);
  const [showFullText, setShowFullText] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Load JSON file on component mount
  useEffect(() => {
    const loadSentenza = async () => {
      try {
        // Try to load from public/data folder (for production)
        const response = await fetch('/data/sentenza_oggi.json');
        if (response.ok) {
          const data: SentenzaData = await response.json();
          setCurrentData(data);
          setError(null);
        } else {
          // Fallback to mock data
          console.log('Using mock data - sentenza_oggi.json not found');
          setCurrentData(mockJudgment);
        }
      } catch (err) {
        console.log('Error loading sentenza_oggi.json, using mock data:', err);
        setCurrentData(mockJudgment);
      } finally {
  ...
  const displayData = currentData || mockJudgment;
  const isMock = currentData?.id === mockJudgment.id || !currentData?.timestamp;
      ...
      <div className="w-full max-w-2xl animate-fade-in">
        <div className="bg-white p-8 md:p-12 shadow-2xl rounded-sm border-t-4 border-gray-900">
          <div className="flex justify-center mb-8">
            <Scale className="w-12 h-12 text-gray-400 animate-pulse" />
          </div>
          <p className="text-center text-gray-600">Caricamento sentenza in corso...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="w-full max-w-3xl animate-fade-in relative">
      ...
      <div className="bg-white shadow-2xl p-12 md:p-20 relative">
        ...
        <div className="text-center border-b-2 border-gray-900 pb-10 mb-10">
          ...
          {(isMock || displayData.timestamp) && (
            <div className="mt-2 text-xs text-green-600 font-bold tracking-widest uppercase">
              {isMock ? 'Sentenza Campione' : 'Aggiornato Automaticamente'}
              {displayData.timestamp && <span> · {new Date(displayData.timestamp).toLocaleDateString('it-IT')}</span>}
            </div>
          )}
        </div>
        ...
        {displayData.keyPoints && displayData.keyPoints.length > 0 && (
          <div className="mt-12 pt-8 border-t border-gray-200">
            <h3 className="font-bold text-gray-900 mb-4 tracking-wide uppercase text-sm">Punti Chiave</h3>
            <ul className="space-y-2">
              {displayData.keyPoints.map((point, idx) => (
                <li key={idx} className="text-gray-700 text-sm flex items-start">
                  <span className="font-bold text-gray-400 mr-3 mt-1">•</span>
                  <span>{point}</span>
                </li>
              ))}
            </ul>
              ...
              {isMock ? (
                <div className="text-center py-12 space-y-6">
                  ...
                  <div className="p-4 bg-gray-100 rounded inline-block text-left text-sm font-mono text-gray-700">
                    <strong>Estremi:</strong><br/><br/>
                    Numero: 6395<br/><br/>
                    Anno: 2025<br/><br/>
                    Data Deposito: 18/09/2025<br/><br/>
                    Valore: € 1.215,63
                  </div>
                  <br/><br/>
                  <a 
